...
typedef struct {
	...
	typedef struct {
		int numSamples;
		short *pData; //with size of numSamples
	} wavedata_t;
	*/
	...
	int location; //must not exceed numSamples
} playbackSound_t;
...
// 
	...
	if(!hasFree) {
		printf("Max soundbites reached\n");
		return;
	}

	soundBites[freeInd].pSound = pSound;
	soundBites[freeInd].location = 0;
	...
	*/
	// printf("%d\n",soundBites[freeInd].pSound->numSamples);
	/*
...
static void fillPlaybackBuffer(short *buff, int size)
{
	...
	*/
	// is this what they mean by synchronization? I feel like no. Ask in OH
	/*
	...
	for(int i = 0; i < MAX_SOUND_BITES; i++) {
		...
		// playbackSound_t cur = soundBites[i];
		...
		soundLoc = soundBites[i].location;
		end = soundBites[i].pSound->numSamples;
		tmp = 0;
		// printf("%ld\n",playbackBufferSize);
		// printf("%d\n",end);
			// printf("true\n");
		...
		for(int j = 0; j < size; j++) {
			...
			tmp = soundBites[i].pSound->pData[soundLoc+j];
			test = tmp + buff[j];

			if(test >= SHRT_MAX) {
				tmp = SHRT_MAX;
			}
			else if(test <= SHRT_MIN) {
				tmp = SHRT_MIN;
			}
			// else {
			buff[j] += tmp;
